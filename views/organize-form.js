...
module.exports = (state, dispatch) => {
  ...
  return html`
    <form method="POST" style="width: 100%;" method="POST" onsubmit=${handleForm(dispatch, { type: 'vote:endorsed', vote })}>
      ...
      <div class="field">
        <div class="control">
          <button class=${`button is-success is-fullwidth has-text-weight-bold fix-bulma-centered-text is-size-5 ${loading.vote ? 'is-loading' : ''}`} disabled=${loading.vote} type="submit">Get involved</button>
        </div>
      </div>
    </form>
  `
